...
ARG* get_args(char const* filename) {
  ARG* result = (ARG*)malloc(sizeof(ARG));
...
int get_num(int a, int b, int c, int d, int e) {
  return abs(a + b * c - d * e);
}

int get_method(char* name) {
  if (strcmp(name, "binary_insertion") == 0) {
    return 0;
  } else if (strcmp(name, "tim") == 0) {
    return 1;
    ...
    return 6;
  } else if (strcmp(name, "rec_stable") == 0) {
    return 7;
  } else if (strcmp(name, "grail") == 0) {
    return 8;
  } else if (strcmp(name, "grail_lazy_stable") == 0) {
    return 9;
...
void init_data(int* data,int* temp,int size){
	for(int i = 0; i<size; i++){
		data[i] = temp[i];
	}
}
...
int main(int argc, char const* argv[]) {
	...
	init_data(data,temp,size);
	cz_binary_insertion_sort(data, size);
	print_data(data,size);


	init_data(data,temp,size);
	cz_tim_sort(data, size);
	print_data(data,size);

	init_data(data,temp,size);
	cz_heap_sort(data, size);
	print_data(data,size);

	init_data(data,temp,size);
	cz_quick_sort(data, size);
	print_data(data,size);

	init_data(data,temp,size);
	cz_merge_sort(data, size);
	print_data(data,size);

	init_data(data,temp,size);
	cz_selection_sort(data, size);
	print_data(data,size);

	init_data(data,temp,size);
	cz_shell_sort(data, size);
	print_data(data,size);

	init_data(data,temp,size);
	cz_rec_stable_sort(data, size);
	print_data(data,size);

	init_data(data,temp,size);
	cz_grail_sort(data, size);
	print_data(data,size);

	init_data(data,temp,size);
	cz_grail_lazy_stable_sort(data, size);
	print_data(data,size);

	init_data(data,temp,size);
	cz_sqrt_sort(data, size);
	print_data(data,size);
